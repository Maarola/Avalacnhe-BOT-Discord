...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.BanMembers)) {
        ...
        .setColor("#752a41")
        .setDescription(`<:aviso:1015652245808877568>  | **O usuário ${user} (\`${user.id}\`) foi desbanido com sucesso!**`);

        let erro = new Discord.EmbedBuilder()
        .setColor("#752a41")
        .setDescription(`<a:x_ava:1002696052421431326> | **Não foi possível desbanir o usuário ${user} (\`${user.id}\`) do servidor!**`);
